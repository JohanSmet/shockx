...
char *fix_sprint_hex (char *str, fix x)
{
	bool neg = FALSE;
	if (x < 0) { x = -x; neg = TRUE; }
	if (!neg) sprintf (str, "%x.%04x", x >> 16, x&0xffff);
	else 		 sprintf (str, "-%x.%04x", x >> 16, x&0xffff);
   return str;
}
...
char *fix24_sprint_hex (char *str, fix24 x)
{
	bool neg = FALSE;
	if (x < 0) { x = -x; neg = TRUE;	}
	if (!neg) sprintf (str, "%x.%02x", x >> 8, x&0xff);
	else      sprintf (str, "-%x.%02x", x >> 8, x&0xff);
   return str;
}
